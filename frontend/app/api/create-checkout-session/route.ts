...
const SWIFTOR_SERVER_URL = process.env.SWIFTOR_SERVER_URL;
...
async function createCheckoutSession(request: NextRequest & { user: User }) {
  ...
  try {
    ...
    const url = `${SWIFTOR_SERVER_URL}/payment${TEST_MODE_ENABLED ? "/test" : ""}/create-checkout-session`;
    ...
    const response = await fetch(url, {
      ...
      body: JSON.stringify({ priceId, userId: user?.id }), // userId fetched from supabase, not from db.json
    });
